...
Laptop MCP adapter is written. `password-manager mcp stdio` against `https://veil.nyc`. Cursor live: `list_items` + `fetch` GitHub `/user` allow/200. Flue Uses Veil on Engineering. One store is written: origin HTTP owns item/grant/list; `PWM_ORIGIN` never opens sqlite. `human login --out-file` is the human mint. Identity on origin is sibling Railway processes (Kratos, Keto, glue, Hydra) plus Cloudflare Workers login UI at `https://login.veil.nyc`. Grants stay in the vault. Chrome URI fill (slice 26) is written: store keepassxc-browser + Go host against origin; `FillEntry.Login` is the sealed username, not item.Name. `totp enroll` writes seed `--out-file` and an otpauth QR `--qr-file`. Human-surface roadmap is SPEC slices 26–37. Next: slice 28 Kratos MFA. Not SPA. Not the Veil extension. Not another sqlite. Infisical backend issues #3–#6 stay after MFA.
