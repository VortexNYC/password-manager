...
7. **Tests first** for grant, inject, and leak. GitHub Actions runs `make ci`. `go test ./...` is the Go suite. The web workspace is `pnpm` and `vp lint` / `vp fmt`. `prove-live` and Chrome for Testing are not CI.
