...
[Aside Password Manager](https://aside.com/features/password-manager) is a proprietary vault inside their Chromium browser ([docs](https://docs.aside.com/help/password-manager.md)). GitHub `aside` is unrelated. There is no code to import. We already listed them in `prior-art.md`. What they *document* (2026-09):
...
Open source we can actually consume: [`apple/password-manager-resources`](https://github.com/apple/password-manager-resources) (MIT). Depend on the JSON. Contribute quirks back. Do not vendor a copy we drift. Do not treat it as field detection.
...
Build order stays **Increments** at the bottom. Do not scaffold `extension` until host JSON `ping` `match` `fill` exist. Do not build `replica` until Chrome execute against origin is proven. Do not build `safari` until Chrome + Firefox work. Generate and passkeys are not a gate on the first JS.
...
- Touch ID in `internal/confirm`. `Host.confirm` reuses 30s globally until cards. CVV never reuses (not in this tree yet).
...
- `openApp` refuses when `PWM_ORIGIN` is set — that footgun stays closed
...
No field catalog. No Bitwarden detector. No kpxc `sites.js`. Matching affiliation is Apple’s MIT `password-manager-resources` (shared-credentials + appIDs), consumed as data, not as a detector. Password generation rules from the same repo (`password-rules.json`) plus the page’s `passwordRules` attribute.

37 fill first does not: HTTP Basic, auto-submit, fill-on-load, clipboard TOTP as the product, default-password-manager prompt, **save-banner for a password the human typed**. Generate-on-new-account is the next layer of 37, not a toy popup.
...
- **Import, one shot.** 1Password `.1pux` / CSV, Apple Passwords, Bitwarden, Chrome. Owner. File in, items on origin. Not Connect. Not sync. Not MCP. SPEC’s “do not become their import” means do not become a vault-sync product. A family will not retype 400 logins. This is the limiter after fill works.
...
`POST /v1/items` is owner today. A family member filling a new Netflix must be allowed to **create a login they own**. Org owner is not the only human who may save. Grants still gate what a member can *fill* of someone else’s. Resolve this on the generate slice, not with a second vault type.
...
4. **Written + CFT-proven with Touch ID. Branded Touch ID not proven.** Thin MV3 in `apps/fill`: hold the port, `match` on nav, choose + execute, write fields, `Cmd-\` remap. Host JSON `need_login` on dead JWT. Chrome 154 branded ignores `--load-extension`. CFT 2026-09-14 `01e723c`: headed fill/passkey/generate/webauthn.io with Touch ID on; Shlomo tapped. Copy that SHA over `~/.password-manager/native-host` — do not `fill install` (it rewrites production `fill.json`). 2026-09-14 copy: fill.json origin `https://veil.nyc`, `touch_id` absent (default on), host process `nyc.veil.fill`. Branded Chrome vortex.nyc: Veil chooser listed `cloudflare-login` and a click wrote the password field. Touch ID sheet was not observed — 1Password also raced the page. That is not a branded Touch ID prove. Agents never `POST /v1/fill/logins`. Do not set `HOME` to a tempdir. Do not `Runtime.evaluate` `fillTab`. Do not `fill install` against production `fill.json`.
5. **Written + CFT-proven with Touch ID.** `passkeyCreate` / `passkeyGet` on `nyc.veil.fill`. Confirm before origin. Cancel fail-closed. UV flag only after that confirm — `userVerification=discouraged` is not a skip. Content intercepts `navigator.credentials.create/get` at `document_start` in MAIN world. Proof: fake origin — create returns attestation, get returns signature, no PEM, deny never POSTs. CFT 2026-09-14: fixture create+get and webauthn.io register→`/profile` with Touch ID on. Branded Chrome passkey on a site that requires UV is not proven. Do not `Runtime.evaluate` create.
6. **Written + CFT-proven with Touch ID.** `generate` on `nyc.veil.fill`. Confirm before mint and `POST /v1/items`. Cancel fail-closed. Nil Confirm is deny. Existing login match is choose only. Page `passwordRules` length and `required` classes; PMR file is later. Chooser name is the host. Proof: fake origin — empty URL never POSTs, deny never POSTs, signup returns host as name, second generate is `choose`. CFT 2026-09-14: `#pass` on `generate-fixture.html` with Touch ID on. Branded generate on a real signup, and chooser “Suggest a password”, are not proven. Live origin is still the 2026-09-11 broker until this SHA deploys.
...
11. Import, then typed-save, TOTP-from-page.
12. Win/Linux native APIs (reopen). Phone when we mean it.
