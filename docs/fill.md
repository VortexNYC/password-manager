...
Build order stays **Increments** at the bottom. Chrome execute against origin is proven (CFT + branded 2026-09-14). Replica is live. Cards + identities are CFT-proven; branded HTML checkout wrote field lengths 2026-09-15. Next is **import onto origin**, then wallet/login create on the owner API. Do not start Firefox or Safari until origin create/import is the way a family leaves 1Password. Firefox is a pipe, not the limiter.
...
9. **Import + owner create. Origin. Not MCP.** One-shot `.1pux` / CSV onto origin (`POST /v1/import`, CLI `item import`). Owner create is `POST /v1/items` with `kind` `api_key` / `card` / `identity`. Card/identity fields are request-only; PAN/CVV/address never in the response, list, or MCP. SPA Add and Import are the same owner calls. Typed-save and TOTP-from-page stay Chrome, after this. **STOP: Firefox / Safari / 32.**
10. Safari `.app` + original Swift. Same JSON.
11. Slice 32 Mac AutoFill.
12. Typed-save and TOTP-from-page (Chrome). Then Win/Linux native APIs (reopen). Phone when we mean it.
