...
4. **Written + proven in Chrome.** Thin MV3 in `apps/fill`: hold the port, `match` on nav, choose + execute, write fields, `Cmd-\` remap. Host JSON `need_login` on dead JWT. Chrome 154 branded ignores `--load-extension` — prove is Chrome for Testing, headed, click `#user`. Native host binds `PWM_HOME` to the binary’s directory. `fill install` writes `nyc.veil.fill` only and removes `org.keepassxc.keepassxc_browser`. Do not set `HOME` to a tempdir. Do not `Runtime.evaluate` `fillTab` on the service worker.
