...
4. **Thin MV3.** Hold the port, `match` on nav from RAM, choose + execute, write fields, `Cmd-\`. Prove Chrome the way we proved kpxc. Dead JWT without replica → `need_login`. **STOP: Chrome fill, online.** Kill the kpxc listing when this is proven.
