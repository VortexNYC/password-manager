...
5. `POST /v1/fill/sync` is live on `pwm` production 2026-09-14 (`bb562d48`). Human 200, 10 rows with material, agent 403. `replica.box` appears on the laptop after the next fill-host ping (Keychain unwrap). Copying the box without Keychain is useless.
