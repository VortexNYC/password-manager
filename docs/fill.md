...
- Same Go binary. Two manifests until customers leave the kpxc listing: `org.keepassxc.keepassxc_browser` (nacl, proof) and `nyc.veil.fill` (JSON, product). Safari never speaks nacl.
...
6. **Host JSON v1 is `ping` `match` `fill`.** Do not block the Chrome wedge on generate or passkeys. Those are later cases on the same dispatcher. Kill the kpxc nacl listing the day Chrome JSON is proven — do not carry two fill protocols.
...
| `host-json` | Native name `nyc.veil.fill`. uint32-LE JSON. v1: `ping` `match` `fill`. Later: `generate` `passkeyCreate` `passkeyGet`. Same Go binary. No nacl. | `kinds`, `confirm` |
...
Cancel = empty fill, fail closed. `PWM_FILL_TOUCHID=0` stays the test hatch.
...
4. **Written + proven in Chrome.** Thin MV3 in `apps/fill`: hold the port, `match` on nav, choose + execute, write fields, `Cmd-\` remap. Host JSON `need_login` on dead JWT. Chrome 154 branded ignores `--load-extension` — prove is Chrome for Testing, headed, click `#user` on `apps/fill/fixture.html`. Native host launched as `native-host` **binds `PWM_HOME` to the binary’s directory**. `fill.json` `touch_id: false` forces Touch ID off. Prove writes `nyc.veil.fill.json` only into Chrome for Testing `NativeMessagingHosts` and removes it; never branded Chrome (kpxc lives there). Do not set `HOME` to a tempdir — CFT then hangs on `Page.navigate`. Do not `Runtime.evaluate` `fillTab` on the service worker. kpxc listing still installed; killing it is a separate cut.
5. **Passkeys** on the JSON dispatcher + intercept. Cuttable.
