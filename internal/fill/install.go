...
func InstallOrigin(env InstallEnv) error {
	...
	if env.Origin != "" {
		cfg.TokenFile = filepath.Join(env.UserHome, ".config/vortex/pwm-human.jwt")
	...
	return os.WriteFile(filepath.Join(ffDir, NativeHostName+".json"), ManifestFirefox(host), 0o644)
}
